...
plt.errorbar(x, y2_mean, yerr=y2_std, fmt='o', ecolor=color2, capsize=5, label='ATP', color=color2)
plt.plot(x, y2_mean, linestyle='-', marker='o', color=color2)

# Plot für Gruppe ATP MgATP
plt.errorbar(x, y3_mean, yerr=y3_std, fmt='o', ecolor=color3, capsize=5, label='ADP', color=color3)
plt.plot(x, y3_mean, linestyle='-', marker='o', color=color3)
# Plot-Details hinzufügen
plt.xlabel('Minuten')
plt.ylabel('nmol')
plt.title('50ug Doppelt')
plt.legend()
plt.grid(True)
